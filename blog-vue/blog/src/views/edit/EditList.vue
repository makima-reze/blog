...
export default {
  ...
  data: function () {
    return {
      ...
      count: 0,
      cover:{
      backgroundImage: "url(" + require("../../assets/fig/editList.gif") + ")",
  ...
  methods: {
    ...
    updateArticleDelete(id) {
      ...
      this.axios.put("/api/articles", param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
    ...
    deleteArticles(id) {
      ...
      this.axios.delete("/api/articles", param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            message: data.message,
    ...
    exportArticles(id) {
      ...
      console.log(param);
      this.axios.post("/api/articles/export", param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            message: data.message,
